...
import { fetchByAdminId, statusFor, getFEFOBatches } from "./shared.js";
...
const demoBatches = [
  {
    productId: "demo-laptop-stand",
    batchNumber: "LS-2026-01",
  ...
  {
    productId: "demo-wireless-mouse",
    batchNumber: "WM-2026-01",
  ...
  {
    productId: "demo-usb-hub",
    batchNumber: "UH-2026-01",
  ...
  {
    productId: "demo-smartwatch",
    batchNumber: "SW-2026-01",
  ...
  {
    productId: "demo-powerbank",
    batchNumber: "PB-2026-01",
...
function takeDemoLock(intervalMs) {
  ...
  try {
    ...
    localStorage.setItem(
      ...
      JSON.stringify({
        owner,
        expiresAt: now + Math.max(intervalMs * 2, 12000),
      }),
    );
    return true;
  } catch {
...
export async function seedDemoProducts({
  ...
  for (const item of demoProducts) {
    const ref = doc(
      db,
      "products",
      `${adminId ? adminId + "-" : ""}${item.id}`,
    );
    const snap = await getDoc(ref);
    if (!snap.exists()) {
      await setDoc(ref, {
        ...item,
        id: `${adminId ? adminId + "-" : ""}${item.id}`,
        source: "demo",
        isDemo: true,
        adminId: adminId,
        adminName: adminName,
  ...
  for (const batch of demoBatches) {
    const batchId = `${adminId ? adminId + "-" : ""}${batch.productId}-${batch.batchNumber}`;
    ...
    if (!batchSnap.exists()) {
      ...
      await setDoc(batchRef, {
        ...batch,
        id: batchId,
        productId: `${adminId ? adminId + "-" : ""}${batch.productId}`,
...
async function getDemoSaleProducts(profile = {}) {
  const adminId = profile?.adminId || profile?.id || null;

  let products = await fetchByAdminId("products", adminId);
  let demo = products.filter((p) => p.source === "demo" || p.isDemo === true);

  if (!demo.length && adminId) {
    await seedDemoProducts({ adminId });
    products = await fetchByAdminId("products", adminId);
    demo = products.filter((p) => p.source === "demo" || p.isDemo === true);
  }

  let available = demo.filter((p) => Number(p.stock || 0) > 0);
  if (!available.length && demo.length) {
    await seedDemoProducts({ restock: true, adminId });
    products = await fetchByAdminId("products", adminId);
    available = products.filter(
      (p) =>
        (p.source === "demo" || p.isDemo === true) && Number(p.stock || 0) > 0,
    );
  }
  return available;
}
...
export async function createDummySale(profile = {}) {
  const products = await getDemoSaleProducts(profile);
  if (!products.length)
    throw new Error("No demo products available. Seed demo products first.");
  ...
  const { selected: fefoBatches, remainingQty } = getFEFOBatches(
    selected.id,
    batches,
    1,
  );
  ...
  await runTransaction(db, async (tx) => {
    ...
    if (!productSnap.exists())
      throw new Error("Selected demo product was not found.");

    const product = productSnap.data();
    const stock = Number(product.stock || 0);
    if (stock <= 0)
      throw new Error(`${product.name || selected.name} is out of stock.`);
    ...
    savedSale = {
      ...
      adminId: adminId,
      adminName: profile?.adminName || profile?.name || "Demo Admin",
      source: "dummy",
      isDemo: true,
...
export function startDemoLiveSales({
  profile = {},
  onSale = null,
  onError = null,
  intervalMs = getDemoInterval(),
} = {}) {
  let stopped = false;
  let busy = false;
  let timer = null;
...
export async function restockRandomProducts({ adminId = null, count = 5 } = {}) {
  if (!adminId) throw new Error('Admin ID required');
  
  const products = await fetchByAdminId('products', adminId);
  if (!products.length) throw new Error('No products found. Add products first.');
  
  const allBatches = await fetchByAdminId('productBatches', adminId);
  const productsWithBatches = products.filter(p => 
    allBatches.some(b => b.productId === p.id && (b.remainingQuantity || 0) > 0)
  );
  
  if (!productsWithBatches.length) throw new Error('No products with batches found. Create batches first.');
  
  let updated = 0;
  const shuffled = [...productsWithBatches].sort(() => 0.5 - Math.random());
  const toUpdate = shuffled.slice(0, Math.min(count, shuffled.length));
  
  for (const product of toUpdate) {
    const productBatches = allBatches.filter(b => b.productId === product.id && (b.remainingQuantity || 0) > 0);
    ...
    await runTransaction(db, async (tx) => {
      ...
      if (!productSnap.exists() || !batchSnap.exists()) return;
      
      const currentStock = Number(productSnap.data().stock || 0);
      const currentRemaining = Number(batchSnap.data().remainingQuantity || 0);
      const newStock = currentStock + randomQty;
      const newRemaining = currentRemaining + randomQty;
      
      // All writes after reads
      tx.update(productRef, {
        stock: newStock,
        status: statusFor(newStock, productSnap.data().minStock || 0),
        updatedAt: serverTimestamp(),
      });
...
export async function createRandomBatches({ adminId = null, adminName = 'Admin', count = 3 } = {}) {
  if (!adminId) throw new Error('Admin ID required');
  ...
  const toCreate = shuffled.slice(0, Math.min(count, shuffled.length));
  
  for (const product of toCreate) {
    const randomQty = Math.floor(Math.random() * 50) + 10;
    const batchNumber = `BATCH-${Date.now().toString().slice(-6)}-${Math.random().toString(36).slice(2, 5).toUpperCase()}`;
    
    const manufactureDate = formatDateForInput(randomDate(new Date('2024-01-01'), new Date()));
    const isPerishable = product.category === 'Food' || product.category === 'Medicine';
    let expiryDate = null;
    
    if (isPerishable) {
      const mfgDate = new Date(manufactureDate);
      const expDate = new Date(mfgDate);
      expDate.setFullYear(expDate.getFullYear() + (product.category === 'Food' ? 1 : 2));
      expiryDate = formatDateForInput(expDate);
    }
    
    const purchasePrice = Math.round((Number(product.costPrice) || Number(product.price) * 0.7) * (0.8 + Math.random() * 0.4));
    ...
    await addDoc(collection(db, 'productBatches'), {
      ...
      manufactureDate,
      expiryDate,
      purchasedQuantity: randomQty,
      remainingQuantity: randomQty,
      supplierName: product.supplierName || 'Auto Supplier',
      purchasePrice,
    ...
    await runTransaction(db, async (tx) => {
      ...
      const currentStock = Number(productSnap.data().stock || 0);
      tx.update(productRef, {
        stock: currentStock + randomQty,
        status: statusFor(currentStock + randomQty, productSnap.data().minStock || 0),
        updatedAt: serverTimestamp(),
      });
    });
