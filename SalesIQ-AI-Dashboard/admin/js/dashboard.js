...
function renderDemoControls() {
  ...
  if (toggle) {
    toggle.textContent = enabled
      ? "Stop Dummy Live Data"
      : "Start Dummy Live Data";
...
$("#seedDemoProducts")?.addEventListener("click", async (e) => {
  ...
  try {
    ...
    toast(
      `Demo products ready. Created: ${result.created}, restocked: ${result.updated}.`,
    );
  } catch (err) {
...
$("#generateBulkSales")?.addEventListener("click", async (e) => {
  const btn = e.currentTarget;
  const count = Math.max(
    1,
    Math.min(
      1000,
      Number($("#bulkSaleCount")?.value || 1),
    ),
  );
  await generateBulkSales(count, btn);
});

// Backwards-compat: older cached HTML still has separate buttons.
...
$("#toggleDemoLive")?.addEventListener("click", () => {
  ...
  toast(
    isDemoLiveEnabled()
      ? "Dummy live data started."
      : "Dummy live data stopped.",
  );
});

async function generateBulkSales(count, btn) {
  setBusy(btn, true, "Generating...");
  ...
  try {
    for (let i = 0; i < count; i++) {
      try {
        await createDummySale(profile);
        created++;
      } catch (err) {
        toast(err.message || "Bulk generation stopped early.", "err");
        break;
      }
    }

    if (created) {
      toast(`${created} dummy sale${created === 1 ? "" : "s"} generated.`);
    }
  } finally {
    setBusy(btn, false);
  }
...
$("#generate10Sales")?.addEventListener("click", (e) =>
  generateBulkSales(10, e.currentTarget),
);

$("#generate100Sales")?.addEventListener("click", (e) =>
  generateBulkSales(100, e.currentTarget),
);

$("#generate1000Sales")?.addEventListener("click", (e) =>
  generateBulkSales(1000, e.currentTarget),
);

// Legacy duplicate: "Restock Inventory" did the same as Seed / Restock.
// Kept for cached HTML; new HTML has only #seedDemoProducts.
$("#generateRestock")?.addEventListener("click", (e) =>
  $("#seedDemoProducts")?.click(),
);
